...
void PangolinWindow::UpdatePointCloudGlobal(
    const std::map<Vec2i, CloudPtr, less_vec<2>> &cloud) {
...
void PangolinWindow::UpdatePointCloudGlobal(
    const std::unordered_map<Eigen::Vector2i, PointCloudPtr,
                             localization::hash_pt2> &map_data) {
  UpdatePointCloudGlobal(sad::convert_map(map_data));
}
...
void PangolinWindow::UpdateNavState(const IMUState &imu_state) {
  UpdateNavState(sad::convert_state(imu_state));
}
